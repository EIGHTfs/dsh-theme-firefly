		...
		function startWallpaper(dock) {
			...
			function render(item) {
				...
				if (item.kind === "video") {
					...
					video.autoplay = true; video.loop = true; video.muted = muted; video.playsInline = true;
			...
			function removeSelected() {
				...
				for (const id of ids) {
					...
					if (!item) continue;
					if (item.custom) {
						// 删除服务器上的壁纸文件（item.data = /theme-firefly-assets/wallpapers/<file>）
						const m = /\/theme-firefly-assets\/wallpapers\/([^?]+)$/.exec(item.data || "");
						if (m) {
			...
			function addWallpaper() {
				...
				input.addEventListener("change", async () => {
					...
					for (const file of files) {
						...
						if (!up || !up.ok) { continue; } // 上传失败跳过该文件（服务器不可达/超限等）
